...
(async () => {
  try {
    ...
    // 1. 元の outputFields に存在する列順で固定
    let header = [...outputFields];

    // 2. すべてのレコードから未知のフィールドを抽出
    records.forEach(record => {
      Object.keys(record).forEach(key => {
        if (!header.includes(key)) {
          header.push(key); // outputFields にない場合は末尾に追加
        }
      });
    });
    ...
    const values = [
      header, // ヘッダ行
      ...records.map(record =>
        header.map(key => normalizeValue(record[key]?.value ?? ''))
      )
    ];
